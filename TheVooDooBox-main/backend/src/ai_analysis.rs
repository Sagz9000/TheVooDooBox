            ...
            let summary = re_summary.captures(&response_text)
                ...
                .unwrap_or_else(|| {
                    ...
                    if let Some(caps) = re_alt_summary.captures(&response_text) {
                        ...
                        // THIRD OPTION: Check if "executive_summary" key exists in a malformed JSON string
                        let re_json_summary = Regex::new(r#"(?is)"executive_summary":\s*"(.*?)"#).unwrap();
                        if let Some(caps) = re_json_summary.captures(&response_text) {
                             caps.get(1).unwrap().as_str().trim().to_string()
                        } else {
                            // Truncate raw response
                            let raw = response_text.trim();
                            if raw.starts_with('{') || raw.starts_with('"') {
                                // Log the raw failure for debugging
                                if let Ok(mut file) = std::fs::OpenOptions::new().create(true).append(true).open("logs/ai_error_raw.log") {
                                    let _ = writeln!(file, "[{}] PARSE FAILURE ({} chars)\nRAW:\n{}\n---\n", Utc::now(), raw.len(), raw);
                                }
