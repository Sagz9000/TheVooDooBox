...
import { AgentEvent, mallabApi } from './mallabApi';
...
export default function ReportView({ taskId, events: globalEvents, onBack }: Props) {
    ...
    useEffect(() => {
        if (taskId) {
            ...
            mallabApi.fetchHistory(taskId, consoleSearch).then(evts => {
                setLocalEvents(evts);
                setLoading(false);
            }).catch(err => {
    ...
    useEffect(() => {
        if (taskId) {
            mallabApi.fetchGhidraFindings(taskId).then(findings => {
            ...
            // Fetch Screenshots
            mallabApi.listScreenshots(taskId).then(shots => {
            ...
            // Fetch AI Report
            mallabApi.fetchAIReport(taskId).then(report => {
    ...
    const handleAIAnalysis = async () => {
        ...
        try {
            ...
            if (taskId) {
                ...
                report = await mallabApi.triggerTaskAnalysis(taskId);
            } else {
                // Session-based analysis (Live)
                report = await mallabApi.getAIAnalysis(events);
            }
            setAiReport(report);
        } catch (error) {
        ...
        <div className="flex flex-col h-full bg-[#050505] text-white overflow-hidden animate-in fade-in duration-300 font-sans">
            ...
            <header className="h-14 border-b border-white/10 bg-[#0a0a0a] flex items-center justify-between px-6 shadow-2xl z-10">
                ...
                <div className="flex items-center gap-4 text-xs font-mono text-zinc-500">
                    ...
                    {aiReport && (
                        <button
                            onClick={() => taskId && mallabApi.downloadPdf(taskId, aiReport)}
                                        ...
                                        {screenshots.map((shot, i) => (
                                            <div key={i} className="group relative rounded-lg overflow-hidden border border-white/10 bg-black/20 hover:border-brand-500/50 transition-all shadow-xl">
                                                <div className="aspect-video relative overflow-hidden bg-black">
                                                    <img
                                                        src={mallabApi.getScreenshotUrl(shot, taskId || undefined)}
                                                        alt={`Screenshot ${i}`}
                                                ...
                                                <div className="p-3 bg-[#111] border-t border-white/5 flex items-center justify-between">
                                                    ...
                                                    <a
                                                        href={mallabApi.getScreenshotUrl(shot, taskId || undefined)}
                                                        target="_blank"
                                        ...
                                        onClick={() => {
                                            ...
                                            if (path) {
                                                if (confirm(`Do you want to PIVOT to a deep-dive binary analysis of: ${path}?`)) {
                                                    mallabApi.pivotBin(path).then(success => {
                                                        if (success) {
