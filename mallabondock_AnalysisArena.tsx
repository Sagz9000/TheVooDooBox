...
import { AgentEvent, mallabApi, BASE_URL } from './mallabApi';
...
export default function AnalysisArena({ target, events, onBack }: Props) {
    ...
    React.useEffect(() => {
        const refreshScreenshots = async () => {
            try {
                const data = await mallabApi.listScreenshots();
    ...
    const handleAIAnalysis = async () => {
        setAiLoading(true);
        try {
            const report = await mallabApi.getAIAnalysis(events);
            setAiReport(report);
        } catch (error) {
            ...
            <header className="h-12 border-b border-security-border bg-security-surface flex items-center justify-between px-6 z-20 shadow-lg">
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        onClick={() => mallabApi.revertVm(target.node, target.vmid)}
                    >
